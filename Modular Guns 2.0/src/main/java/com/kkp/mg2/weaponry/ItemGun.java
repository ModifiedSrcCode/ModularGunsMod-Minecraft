...
import com.kkp.mg2.weaponry.entity.projectile.EntityBullet;
...
public class ItemGun extends Item {
	...
	public ItemGun(int aA, float r, float fD, float rT, float d, float zA)
	{
		this.ammoAmount = aA;
		this.recoil = r;
		this.fireDelay = fD;
	...
	public void onUpdate(ItemStack stack, World world, Entity entity, int i, boolean isSelected)
	{
		if(entity instanceof EntityPlayer && ((EntityPlayer)entity).inventory.getCurrentItem() == stack) 
		{
			...
			if(isShootButtonDown)
			{
				...
				player.cameraPitch = recoil - recoil * 3.75F;
			}
			...
			if(currentRecoil > 0) {
				Minecraft.getMinecraft().thePlayer.renderArmPitch -= currentRecoil * 5F;
				Minecraft.getMinecraft().thePlayer.renderArmYaw += currentRecoil * 2.5F;
